...
public class DealgifierIOTalonFX implements DealgifierIO {
  ...
  private final TrapezoidProfile armProfile =
      new TrapezoidProfile(
          new TrapezoidProfile.Constraints(
              Units.degreesToRadians(10), Units.degreesToRadians(100)));
  private TrapezoidProfile.State lastState = new TrapezoidProfile.State();

  private final LinearSystem<N2, N1, N2> armSystem =
      LinearSystemId.createSingleJointedArmSystem(DCMotor.getKrakenX60(1), 100, 50);
  ...
  private final LinearSystemLoop<N2, N1, N1> armLoop =
      new LinearSystemLoop<>(
          (LinearSystem<N2, N1, N1>) armSystem.slice(0), armController, armObserver, 12.0, 0.020);
  ...
  public void reset() {
    armLoop.reset(
        VecBuilder.fill(
            armMotor.getPosition().getValueAsDouble(), armMotor.getVelocity().getValueAsDouble()));
